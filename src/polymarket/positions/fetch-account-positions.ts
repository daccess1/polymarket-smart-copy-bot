...
export async function fetchAccountPositions(address: string) {
    const response = await axios.get<AccountPosition[]>(`https://data-api.polymarket.com/positions?sizeThreshold=1&limit=1000&sortBy=INITIAL&sortDirection=DESC&user=${address}`);
